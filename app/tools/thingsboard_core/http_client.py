...
class ThingsboardSession:
    ...
    def login(self, username: str, password: str) -> str:
        """Executes POST /api/auth/login and stores JWT."""
    ...
    def set_token(self, token: str) -> None:
        """Explicitly set an auth token (e.g. for impersonation)."""
        with self._lock:
            self._token = token

    def refresh(self) -> str:
        """Refreshes the token using refresh token or stored credentials."""
        with self._lock:
...
class ThingsboardHttpClient:
    ...
    def post(self, path: str, data: Any = None, params: Optional[dict] = None) -> Any:
        return self.request("POST", path, data=data, params=params)

    def delete(self, path: str, params: Optional[dict] = None) -> bool:
        try:
    ...
    def paginated_get(self, path: str, page_size: int = 100, extra_params: Optional[dict] = None) -> list[dict]:
        items: list[dict] = []
        page = 0
